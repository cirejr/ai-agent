...
test("parses a text-only response", () => {
  const result = OpenAIResponses.fromProvider(resp01.response, Model.make({ id: "gpt-4o", provider: "openai"}))
  if (!result.ok) throw new Error(result.error.message)
  expect(result.value).toEqual(resp01.expected)
})

test("folds reasoning and text into a single assistant message", () => {
  const result = OpenAIResponses.fromProvider(resp02.response, Model.make({ id: "gpt-4o", provider: "openai"}))
  if (!result.ok) throw new Error(result.error.message)
  expect(result.value).toEqual(resp02.expected)
})

test("parses text, tool call and tool output with correct ordering", () => {
  const result = OpenAIResponses.fromProvider(resp03.response, Model.make({ id: "gpt-4o", provider: "openai"}))
  if (!result.ok) throw new Error(result.error.message)
  expect(result.value).toEqual(resp03.expected)
})

test("maps a failed tool output to an error result", () => {
  const result = OpenAIResponses.fromProvider(resp04.response, Model.make({ id: "gpt-4o", provider: "openai"}))
  if (!result.ok) throw new Error(result.error.message)
  expect(result.value).toEqual(resp04.expected)
})

test("rejects malformed tool call arguments", () => {
  const result = OpenAIResponses.fromProvider(resp05.response, Model.make({ id: "gpt-4o", provider: "openai"}))
  if (result.ok) throw new Error("expected an error")
  expect(result.error._tag).toBe(resp05.expectedError._tag)
})

test("maps a provider error to an AppError", () => {
  const result = OpenAIResponses.fromProvider(resp06.response, Model.make({ id: "gpt-4o", provider: "openai"}))
  if (result.ok) throw new Error("expected an error")
  expect(result.error).toEqual(resp06.expectedError)
})

test("handles an empty output", () => {
  const result = OpenAIResponses.fromProvider(resp07.response, Model.make({ id: "gpt-4o", provider: "openai"}))
  if (!result.ok) throw new Error(result.error.message)
